...
export const WorkoutHistory = ({ workouts, excludeDate }: WorkoutHistoryProps) => {
  ...
  if (items.length === 0) {
    return <p className="text-sm text-slate-500 dark:text-slate-300">No previous workouts logged.</p>
  }
    ...
    <ul className="divide-y divide-slate-100 border border-slate-400">
      {items.map((workout) => (
        <li key={workout.id} className="flex items-center justify-between px-3 py-2 text-sm">
          <span className="text-slate-500 dark:text-slate-300">{formatDate(workout.date)}</span>
          <span className="font-medium text-slate-950 dark:text-slate-300">{WORKOUT_LABELS[workout.type]}</span>
        </li>
      ))}
    </ul>
  )
